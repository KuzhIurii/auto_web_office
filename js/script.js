...
$(function () {
    ...
    function formValide() {
        var str = $('#contact_form input[name=name]').val();
        str = jQuery.trim(str);
        if (str.length < 1) {
            alert('Представтесь пожалуйста');
            return false;
        }
        return true;
    }
    ...
    $('#contact_form .button').live('click', function () {
        if (formValide()) {
            ...
            $('#contact_form').before('<h3 id="contact_form_info">Отправвляем данные. Подождите...</h3>');
            $('#contact_form').hide();
            ...
            $.post(order_url, {
                ...
                comment: $('#contact_form textarea[name=comment]').val(),
            }, function (data) {
